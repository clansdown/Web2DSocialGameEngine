  ...
  async function getTdData(): Promise<TDRoundKickoffResponse> {
    const pending = await getConfig<any>('pending_custom_game');
    if (pending) {
      await deleteConfig('pending_custom_game');
      return pending as TDRoundKickoffResponse;
    }
    return await tdRound(characterId, { mini_game: miniGame, level_id: levelId }) as TDRoundKickoffResponse;
  }

  async function initEngineWithData() {
